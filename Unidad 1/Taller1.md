# Taller 1
## Creamos el repositorio:  
### Se crea con el comando mkdir seguido del nombre dentro de la carpeta repositorios.
![Repositorio](img/crearrepositorio.png)

## Repositorio desde GitHub:
![RepositorioHub](img/repositoriogithub.png)

## Ver el contenido del fichero .git/config:
![Fichero](img/contenidofichero.png)

## Ver los commits que has realizado:
![Commits](img/commits.png)

## Creamos repositorio prueba2_Julian
![Repositorio2](img/repositorio2.png)


## Creamos archivo
![Archivo](img/creararchivo.png)

##
![Archivo](img/creararchivo4.2.png)

## Repositorio remoto
![Remoto](img/punto4.2.png)

## Comprobamos que está bien
![Comprobar](img/Comprobar.png)
